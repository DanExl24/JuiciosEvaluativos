  ...
  <div v-else class="grid gap-6">
    ...
    <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
      ...
      <button 
        ...
        class="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-slate-950/15 transition hover:-translate-y-0.5 hover:bg-blue-700"
      >
    ...
    <div v-else class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      <div v-for="project in projects" :key="project.id_proyecto" class="flex flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/50">
        <div class="border-b border-slate-100 bg-gradient-to-br from-slate-50 to-slate-100/50 p-6">
          ...
          <h3 class="mt-4 line-clamp-2 min-h-[3.5rem] text-lg font-black leading-tight tracking-tight text-slate-900" :title="project.proyecto_nombre">
            {{ project.proyecto_nombre }}
          </h3>
          <p class="mt-3 text-[0.65rem] font-bold uppercase tracking-wider text-slate-500">Programa: <span class="text-slate-700">{{ project.programa_codigo }}</span></p>
        </div>
        ...
        <div class="mt-auto border-t border-slate-50 bg-slate-50/50 p-4">
          <button 
            @click="openProject(project)"
            class="flex w-full items-center justify-center gap-2 rounded-xl bg-white border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
          >
      ...
      <div class="w-full max-w-2xl overflow-hidden rounded-[2rem] border border-white/20 bg-white/95 shadow-2xl backdrop-blur-xl">
        ...
        <div class="p-6">
          ...
          <div
            ...
            :class="isDragActive ? 'border-blue-500 bg-blue-50/50' : 'border-slate-200 bg-slate-50/50 hover:border-blue-300'"
          ...
          >
            <div class="flex min-h-[16rem] flex-col items-center justify-center px-6 py-8">
              <div class="space-y-4 text-center">
                <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-base font-black text-white shadow-lg shadow-blue-600/20">
                  PDF
                </div>
                <div class="space-y-1">
                ...
                <label class="inline-flex cursor-pointer items-center justify-center rounded-xl bg-slate-950 px-5 py-2.5 text-sm font-semibold text-white shadow-lg transition hover:bg-blue-700">
                  Seleccionar PDF
